# Checklist: Lançamento de Campanha — EMB

> Responsável: @estrategista-emb
> Usar ANTES de ativar qualquer campanha

## Fundação
- [ ] Briefing aprovado
- [ ] Estratégia aprovada pelo dono
- [ ] Oferta estruturada e validada (Hormozi)

## Copy (@copywriter-emb)
- [ ] Todas as peças de copy revisadas (score >=80%)
- [ ] Tom de voz do expert consistente
- [ ] CTAs claros em todas as peças
- [ ] Sequência de emails pronta
- [ ] Zero expressões da blacklist anti-IA

## Páginas (@diretor-criativo-emb)
- [ ] Página de captura testada (desktop + mobile)
- [ ] Página de vendas testada (desktop + mobile)
- [ ] Checkout funcionando (teste de compra real)
- [ ] Páginas de obrigado configuradas
- [ ] Velocidade de carregamento OK (<3s)
- [ ] Plataforma de destino otimizada (/page-optimizer)

## Tráfego (DONO executa / [F] @gestor-trafego-emb)
- [ ] Campanhas criadas e configuradas
- [ ] Públicos definidos (frio, morno, quente)
- [ ] Pixel instalado e disparando eventos
- [ ] Budget distribuído conforme plano

## Automações (DONO executa / [F] @automacao-emb)
- [ ] Sequências de email testadas
- [ ] WhatsApp/chatbot testado
- [ ] Integrações funcionando (CRM, pagamento, email)
- [ ] Lembretes agendados (se evento)

## Tracking (DONO executa / [F] @dev-emb)
- [ ] Pixel Meta instalado e testado
- [ ] Google Ads tag configurada (se aplicável)
- [ ] UTMs nos links
- [ ] Eventos de conversão configurados

## Aprovação Final
- [ ] @estrategista-emb aprovou todas as peças
- [ ] Teste ponta a ponta do funil completo
- [ ] Dono deu GO explícito